...
export function getHTML5TagProvider() {
    ...
    var result = []     // [{attr:, prop:, tags:[]}]

    for(let tagName of Object.getOwnPropertyNames(HTML_TAGS)){
        let tagAttributes = HTML_TAGS[tagName].attributes;
        let attributes = globalAttributes.concat(tagAttributes);

        let c = document.createElement(tagName);        
        let pt = c.constructor.prototype;
        //let props = Object.getOwnPropertyNames(pt);
        ...
        for(var attr of attributes){
            ...
            let global = (globalAttributes.indexOf(attr) > -1);
            attr = attr.replace(/:([a-z]+)$/, '');
            let a = result.find(v => v.attr == attr);            
            if(!a) {
                a = {attr: attr, tags : [], global: global}; 
                result.push(a);
            }
            let prop = findProp(attr, props);            
            a.prop = prop;
            a.tags.push(tagName);
        }
    }
    console.table(result.map(item => {return {
                    'attr': item.attr, 
                    'prop': item.prop,
                    'global': item.global,                    
                    'tags': item.tags.join(), 
    ...
    let r2 = result.map(item => {return {
            'attr': item.attr, 
            'prop': item.prop,
            'global': item.global,                    
            'tags': item.global? null: item.tags, 
        };
    })//.filter(item => item.prop == null);
    console.log(JSON.stringify(r2));
}

function findProp(attr, props){
    for(let p of props){
        if(p == attr || p.toUpperCase() == attr.toUpperCase() || attr.replace(/-/g, '').toUpperCase() == p.toUpperCase()){
            return p;
        }
    }
    /*
