...
import "../jsondiffpatch/jsondiffpatch.umd.js";
...
export class Molecule {
    ...
    * leaved - 删除后发生
    ...
    createOutsideStyle(styleDef){
        const key = styleDef.key;
        if(document.head.querySelector(`link[id='${key}']`)) return;        // exists

        const b = new Blob([styleDef.props.innerHTML], {type: 'text/css'});
    ...
    install(plugin){
        if(this.plugins && this.plugins.indexOf(plugin) != -1) return;
        if(this.plugins == null) this.plugins = [];
        this.plugins.push(plugin);
        plugin.molecule = this;
        const handlers = ['willInit', 'willRender', 'willFill', 'filled', 'rendered', 'inited', 'willEnter', 'entered', 'swap', 'willLeave', 'leaved']
        const events = ['willinit', 'willrender', 'willfill', 'filled', 'rendered', 'inited', 'willenter', 'entered', 'swap', 'willleave', 'leaved']
        for(let i=0; i<handlers.length; i++){
            const h = handlers[i];
            if(plugin[h]){
                this.element.addEventListener(events[i], plugin[h].bind(plugin));
            }
        }
        return this;
    }
...
Molecule.extends = function(fullname, subclass){
    let defaultProps = [];
    ...
    Molecule.TYPES[fullname] = subclass;
}
...
export class Prop{
    ...
    getValue(_this){
        if(this.type == 'evt'){     // this.expression will be a function to return function, function(){return this.handleClick} etc
            if(this.expression instanceof EventHandlerProvider)
