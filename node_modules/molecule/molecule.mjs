...
async function loadHtml(res) {
    ...
    await scanDefines(dom, link.href);	// this is a promise
    return true;
}
...
async function scanDefines(starter, baseUrl) {
    for(var template of Array.prototype.slice.call((starter || document).querySelectorAll('template'))){
        ...
        for(var el of Array.prototype.slice.call(template.content.querySelectorAll('[m-def]'))) {
            await collectDefine(el, baseUrl);
            el.remove();
            found = true;
        };
...
async function collectDefine(prototypeElement, baseUrl){
    ...
    try {
        ...
        let code = await compileDefine(prototypeElement, fullname);
...
async function compileDefine(prototypeElement, fullname){
    let unit = new Unit();
...
document.addEventListener('DOMContentLoaded', async function(){
	...
	await scanDefines(document, document.baseURI);
	await scanMolecules();
