import {Expr, ArrayLiteralExpr, AttachEventExprStmt, BracketExpr, AssignStmt, ClassDecl, ConcatStringExpr, ConstDeclStmt, ConstructorDecl,
        ...
        PropAssignExprStmt, Statement, ReturnStmt, SwitchStmt, Unit, VarDeclStmt, ImportStmt, AwaitExpr, CreateElementExpr, OrExpr, Comment
    } from './codedom.js';
...
if(!Function.emptyFn) Function.emptyFn = function(){}
if(!Function.debugger) Function.debugger = function(){debugger; return true;}
...
async function scanDefines(starter, baseUrl, includeAllResources=false, pathes) {
    const unit = new Unit();
    unit.children.push(new Comment(`FROM ${baseUrl}`, true));
...
async function compileDefine(prototypeElement, fullname, unit=null){
    ...
    if(unit == null) {
        unit = new Unit();
        unit.children.push(new Comment(`FROM ${document.URL}`, true));
    ...
    function parseAttributeValue(value, type, isExpr, isInstancing){
        if(type == 'evt'){
            if(isExpr){
                if(!isInstancing){
                    let fun = new FunctionDeclExpr('', ['$c'], [new ReturnStmt(new Expr(value))]);
                    ...
                    return new MethodInvokeExpr('this', 'wrapHandler', [new Expr(value), new Expr('$c')]);
                }
            } else {
                if(!isInstancing){
                    ...
                    let fun = new FunctionDeclExpr('', ['event', 'target', '$c'], [new LineStmt(value)]);
                    return new MethodInvokeExpr('this', 'wrapHandler', [fun, new Expr('$c')]);
                }
            }
        }
        if(isExpr){     // expr
        ...
        for(let child of children){
            ...
            if(child instanceof HTMLElement){
                for(let cattr of child.attributes){
                    ...
                    if(isRuntime) throw `:r(untime) option can only appear within molecule define,  ${cattr.name}=${cattr.value} found`;
            ...
            if(tagName == 'if'){
                ...
                const orExpr = new OrExpr(new FunctionInvokeExpr(funcName,[]), new Expr('[]'));
                array.push(new ExpandIteratorExpr(new BracketExpr(orExpr)));
                continue;
            } else if (tagName == 'for'){
...
async function scanMolecules(starter, manual) {
    ...
    async function createMolecule(target){
        ...
        function createMoleculeWithoutInnerClass(target) {
            ...
            let props = {innerHTML: target.innerHTML};
            var key = null;
            for(let cattr of target.attributes){
                let value = cattr.value;
