...
// import "./codedom.js'; // not work
// import * from "./codedom.js'; // not work
import {Expr, ArrayLiteralExpr, AttachEventExprStmt, BracketExpr, AssignStmt, ClassDecl, ConcatStringExpr, ConstDeclStmt, ConstructorDecl,
        ...
        PropAssignExprStmt, Statement, ReturnStmt, SwitchStmt, Unit, VarDeclStmt, ImportStmt, AwaitExpr
        } from './codedom.js';
...
async function loadHtml(res) {
    ...
    await scanDefines(dom, link.href, true);	// this is a promise
    return true;
}
 ...
 * @param starter 
 * @param baseUrl 
 * @param compilePage 是否编译全页面。对于 loadHtml 指定为 true。此开关开启将全页面引用的 script 作为生成项引入。
 */
async function scanDefines(starter, baseUrl, compilePage=false) {
    const unit = new Unit();

    const importContextEl = document.head.querySelector('meta[name=import_context]');
    const importContextPath = '..' || (importContextEl && importContextEl.getAttribute('value'));
    const importModulesEl = document.head.querySelector('meta[name=node_modules]');

    unit.children.push(new ImportStmt(['Molecule', 'EventHandlerProvider'], 'molecule/molecule-comp.js', ));
    
    for(var template of Array.of(...(starter || document).querySelectorAll('template'))){
        var found = false;
        // for(var el of Array.of(...document.querySelectorAll('script[src]'))){
        //     unit.
        // }

        for(var el of Array.of(...template.content.querySelectorAll('[m-def]'))) {
...
async function collectDefine(prototypeElement, baseUrl, unit){
    ...
    try {
        ...
        var asyncScripts = [];
        csses.concat(moleculeRefs).forEach(resEle => {
        	var append = true;
        	var isCss = (resEle.tagName == 'LINK');
        	var attr = 'src';
        	if(isCss) attr = 'href';
        	var src = resEle.getAttribute(attr);
        	if(src){
        		if(resEle.tagName != 'MOLECULE' && baseUrl && src != resEle[attr]){
            		var abs = absolute(baseUrl, resEle.getAttribute(attr));
            		resEle.setAttribute(attr, abs);
            		src = resEle[attr]
            	}
        		if(_LOAD_ONCE_RESOURCE[src] == null){
        			_LOAD_ONCE_RESOURCE[src] = 1
        		} else {
        			append = false;
        		}
        	}
            if(append) {
            	var clone = resEle.cloneNode(true);
            	if(src && (resEle.tagName == 'SCRIPT' || resEle.tagName == 'MOLECULE')){
        ...
        let code = await compileDefine(prototypeElement, fullname, unit);
    	...
    	return new Promise(async function next(resolve, reject){
    		...
    		if(script.tagName == 'MOLECULE'){
				...
				script.onload = function(){
					if(DEBUG) console.log(this.src + ' loaded')
					next(resolve, reject);
	    		}
...
async function compileDefine(prototypeElement, fullname, unit=null){
    ...
    if(unit == null) {
        unit = new Unit();
        unit.children.push(new ImportStmt(['Molecule', 'EventHandlerProvider'], 'molecule/molecule-comp.js', ));
        //unit.children.push(new ConstDeclStmt('{Molecule, EventHandlerProvider}', new AwaitExpr(new FunctionInvokeExpr('dimport', ['molecule-comp.js']))));
    }
    let c = new ClassDecl(className, prototypeElement.extends || 'Molecule')
    ...
    unit.children.push(new LineStmt(`Molecule.extends(${JSON.stringify(fullname)}, ${className})`));
        ...
        if(type == 'evt'){
            if(isExpr){
                if(!isInstancing){
                    ...
                    return new NewInstanceExpr('EventHandlerProvider', [fun]);
                } else {
...
async function scanMolecules(starter, manual) {
    ...
    async function createMolecule(target){
        if(!createMoleculeWithoutInnerClass(target)){     // 尽量不创建内部类
            ...
            const code = await compileDefine(target, className); 
...
document.addEventListener('DOMContentLoaded', async function(){
	...
	await scanDefines(document, document.baseURI, false);
	await scanMolecules();
